...
public static class StartupAppService
{
    ...
    private static readonly string[] RegistrationNames = ["Pandora", "OrbitDock"];
    private static readonly byte[] EnabledStartupApprovedValue = [0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];

    /// <summary>
    /// A disabled or legacy registration still exists and must not be repaired
    ...
    public static void SetEnabled(bool enabled, string? iconStyle = null)
    {
        ...
        foreach (var registration in registrations)
        {
            ...
            if (registration.HasShortcut && (!enabled || !string.Equals(registration.Name, BrandIdentity.Name, StringComparison.Ordinal)))
            {
    ...
    private static HashSet<string> GetOwnedExecutablePaths(LaunchInfo launch)
    {
        ...
        foreach (var name in new[] { "Pandora.App.exe", "OrbitDock.App.exe" })
        {
            targets.Add(Path.GetFullPath(Path.Combine(appDirectory.FullName, name)));
        }
        // The portable rebrand moves between these two sibling output folders.
        // Do not search other checkouts or infer ownership from filenames alone.
        if (appDirectory.Parent is not null &&
            (string.Equals(appDirectory.Name, "Pandora-win-x64", StringComparison.OrdinalIgnoreCase) ||
             string.Equals(appDirectory.Name, "OrbitDock-win-x64", StringComparison.OrdinalIgnoreCase)))
        {
            targets.Add(Path.Combine(appDirectory.Parent.FullName, "Pandora-win-x64", "Pandora.App.exe"));
            targets.Add(Path.Combine(appDirectory.Parent.FullName, "OrbitDock-win-x64", "OrbitDock.App.exe"));
        }
        return targets;
    }
