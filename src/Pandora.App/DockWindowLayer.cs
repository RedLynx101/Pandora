...
public static class DockWindowLayer
{
    ...
    private static bool IsLayerAnchorWindow(IntPtr handle)
    {
        ...
        var extendedStyle = GetWindowLongPtr(handle, GwlExStyle).ToInt64();
        if (IsPandoraOverlay(handle, extendedStyle) || IsWindowCloaked(handle))
        {
            return false;
        }
    ...
    private static bool IsUserApplicationWindow(IntPtr handle)
    {
        ...
        if ((style & WsVisible) == 0 ||
            ...
            IsPandoraOverlay(handle, extendedStyle) ||
    ...
    private static bool IsPandoraOverlay(IntPtr handle, long extendedStyle)
    {
