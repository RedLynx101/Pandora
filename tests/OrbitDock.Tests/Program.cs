...
{
    var tempPath = Path.Combine(Path.GetTempPath(), "OrbitDock.Tests", Guid.NewGuid().ToString("N"), "workspace.json");
...
{
    var root = Path.Combine(Path.GetTempPath(), "OrbitDock.Tests", Guid.NewGuid().ToString("N"), "Music");
...
static string FindRepoRoot()
{
    ...
    {
        if (File.Exists(Path.Combine(current.FullName, "OrbitDock.sln")))
        {
