...
```powershell
dotnet run --project tests/Pandora.App.Tests -c Release -- --output C:\absolute\task\work\ui-evidence
```
...
Bar-size coverage spans three structures and three sizes with top/bottom anchoring, open/closed state, one/six tabs and narrow dock renders. It checks title/control sizing, single-line names, hidden collapsed tabs, preserved tab identity/selection/content, horizontal overflow width and scrolling, remembered expanded geometry, live preview, persistence, close/revert and failed-save rollback.
